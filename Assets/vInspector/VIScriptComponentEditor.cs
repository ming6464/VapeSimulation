    ...
    class VIScriptComponentEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            void drawBody()
            {
                ...
                {
                    if (!selectedTabPath.StartsWith(drawingTabPath))
                        return;
                    ...
                    void drawSubtabs(Tab tab)
                    {
                        ...
                        var selName = TabsMultiRow(tab.selectedSubtab.name, false, 24,
                                tab.subtabs.Select(r => r.name).ToArray());
                ...
                void drawCurProperty()
                {
                    ...
                    void findFieldInfo()
                    {
                        ...
                        while (fieldInfo == null && curType != null && curType != typeof(MonoBehaviour)
                            && curType != typeof(ScriptableObject))
                            if (curType.GetField(curProperty.name,
                                        BindingFlags.NonPublic | BindingFlags.Public
                                                               | BindingFlags.Instance) is FieldInfo fi)
                                fieldInfo = fi;
                            else
                    ...
                    void ifs()
                    {
                        ...
                        if (ifAttribute == null)
                        {
                            ...
                            if (ifAnyAttribute is HideIfAnyAttribute)
                                hide = ifAnyAttribute.Evaluate(target);
                            if (ifAnyAttribute is ShowIfAnyAttribute)
                                hide = !ifAnyAttribute.Evaluate(target);
                            if (ifAnyAttribute is DisableIfAnyAttribute)
                                disable = ifAnyAttribute.Evaluate(target);
                            if (ifAnyAttribute is EnableIfAnyAttribute)
                                disable = !ifAnyAttribute.Evaluate(target);
                        }
                        else
                        {
                            if (ifAttribute is HideIfAttribute)
                                hide = ifAttribute.Evaluate(target);
                            if (ifAttribute is ShowIfAttribute)
                                hide = !ifAttribute.Evaluate(target);
                            if (ifAttribute is DisableIfAttribute)
                                disable = ifAttribute.Evaluate(target);
                            if (ifAttribute is EnableIfAttribute)
                                disable = !ifAttribute.Evaluate(target);
                        }
                    ...
                    void foldouts()
                    {
                        ...
                        for (; sharedLength < newPathSplit.Length && sharedLength < drawingPathSplit.Length;
                             sharedLength++)
                        ...
                        {
                            if (!data.rootFoldout.IsSubfoldoutContentVisible(drawingFoldoutPath))
                                break;
                    ...
                    void drawGreenDividingLine()
                    {
                        var drawGreenDividingLineAttribute = fieldInfo.GetCustomAttribute<DrawGreenDividingLine>();

                        if (drawGreenDividingLineAttribute != null)
                        {
                            if (drawGreenDividingLineAttribute.TopSpacing > 0)
                            {
                                GUILayout.Space(drawGreenDividingLineAttribute.TopSpacing);
                            }

                            var rect = EditorGUILayout.GetControlRect(false, 2);
                            EditorGUI.DrawRect(rect, Color.green);

                            if (drawGreenDividingLineAttribute.BottomSpacing > 0)
                            {
                                GUILayout.Space(drawGreenDividingLineAttribute.BottomSpacing);
                            }
                        }
                    }
                    ...
                    if (fieldInfo == null)
                        return;
                    if (fieldInfo.FieldType == typeof(VInspectorData))
                        return;
                    ...
                    if (selectedTabPath != drawingTabPath && !selectedTabPath.StartsWith(drawingTabPath + "/"))
                        return;
                    ...
                    drawGreenDividingLine();

                    if (fieldInfo.GetCustomAttribute<ButtonAttribute>() != null)
                        return;
            ...
            void drawButtons()
            {
                ...
                {
                    if (button.tab != "" && !selectedTabPath.StartsWith(button.tab))
                        continue;

                    var ifAttribute    = button.ifAttribute;
                    var ifAnyAttribute = button.ifAnyAttribute;

                    if (ifAttribute is HideIfAttribute && ifAttribute.Evaluate(target))
                        continue;
                    if (ifAttribute is ShowIfAttribute && !ifAttribute.Evaluate(target))
                        continue;
                    if (ifAnyAttribute is HideIfAnyAttribute && ifAnyAttribute.Evaluate(target))
                        continue;
                    if (ifAnyAttribute is ShowIfAnyAttribute && !ifAnyAttribute.Evaluate(target))
                        continue;

                    var prevGuiEnabled = GUI.enabled;
                    if (ifAttribute is DisableIfAttribute && ifAttribute.Evaluate(target))
                        GUI.enabled = false;
                    if (ifAttribute is EnableIfAttribute && !ifAttribute.Evaluate(target))
                        GUI.enabled = false;
                    if (ifAnyAttribute is DisableIfAnyAttribute && ifAnyAttribute.Evaluate(target))
                        GUI.enabled = false;
                    if (ifAnyAttribute is EnableIfAnyAttribute && !ifAnyAttribute.Evaluate(target))
                        GUI.enabled = false;
        ...
        {
            var serializedDataField = target.GetType().GetFields(maxBindingFlags)
                                            .FirstOrDefault(r => r.FieldType == typeof(VInspectorData));

            void getCached()
            {
                if (!VInspectorCache.instance.datas_byTarget.ContainsKey(target))
                    return;
            ...
            void markTargetDirty()
            {
                ...
                if (serializedDataField == null)
                    return;
                if (serializedDataField.GetValue(target) != null)
                    return;
        ...
        {
            if (target)
                scriptMissing = target.GetType() == typeof(MonoBehaviour)
                             || target.GetType() == typeof(ScriptableObject);
            else
